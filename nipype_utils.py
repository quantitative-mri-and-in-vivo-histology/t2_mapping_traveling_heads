...
class BidsOutputFormatterInputSpec(BaseInterfaceInputSpec):
    ...
    output_dir = traits.Directory(
        desc="Directory where the renamed file will be saved (optional)")
...
class BidsOutputFormatter(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        output_dir = self.inputs.output_dir if isdefined(
            self.inputs.output_dir) else runtime.cwd
...
class BidsOutputWriterInputSpec(BaseInterfaceInputSpec):
    ...
    json_dict = traits.Dict(desc="Dictionary to be saved as a JSON file", mandatory=False)
    entity_overrides = traits.Dict(desc="Dictionary of tags to override in file entities", mandatory=False)
    pattern = traits.Str(mandatory=True, desc="Pattern for output filename")
    output_dir = traits.Directory(desc="Directory where the renamed file will be saved", mandatory=True)
...
# Define the interface for renaming the file based on BIDS metadata
class BidsOutputWriter(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        filename = layout.build_path(file_entities, validate=False,
                                     path_patterns=[self.inputs.pattern], absolute_paths=False)
    ...
    def _list_outputs(self):
        outputs = self._outputs().get()
        outputs['out_file'] = self._out_file  # Return the filename generated in _run_interface
        if self.inputs.json_dict:
            outputs['out_json'] = self._out_json  # Return the JSON filename if generated
        return outputs