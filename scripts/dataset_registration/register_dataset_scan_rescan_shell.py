...
class PairwiseRegistration(CommandLine):
    ...
    def _list_outputs(self):
        ...
        paths_a = construct_output_paths(self.inputs.moving_image_a, 0)
        paths_b = construct_output_paths(self.inputs.moving_image_b, 1)

        outputs['template_image'] = os.path.join(base_dir,
                                                 f"{output_prefix}template0.nii.gz")
        outputs['affine_transform_a'] = paths_a["affine"]
        outputs['forward_warp_a'] = paths_a["forward_warp"]
        outputs['inverse_warp_a'] = paths_a["inverse_warp"]
        outputs['warped_to_template_a'] = paths_a["warped_image"]

        outputs['affine_transform_b'] = paths_b["affine"]
        outputs['forward_warp_b'] = paths_b["forward_warp"]
        outputs['inverse_warp_b'] = paths_b["inverse_warp"]
        outputs['warped_to_template_b'] = paths_b["warped_image"]
